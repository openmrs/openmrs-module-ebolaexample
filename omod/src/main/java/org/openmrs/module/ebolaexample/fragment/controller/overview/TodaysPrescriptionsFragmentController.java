...
public class TodaysPrescriptionsFragmentController {
    ...
    public void controller(@FragmentParam("patient") PatientDomainWrapper patient, FragmentModel model) {
        ...
        List<DrugOrder> todaysDrugOrders = new ArrayList<DrugOrder>();
        for (Order order : orders) {
            if(DateUtil.isInLast24Hours(order.getDateCreated())) {
                todaysDrugOrders.add((DrugOrder) order);
            }
        }

        model.addAttribute("todaysDrugOrders", todaysDrugOrders);
    }
}
